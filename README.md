...
`tuimsg` sends only the characters that changed on screen, and each update leaves as a single write, so a keystroke typically costs tens to a few hundred bytes. Moving through the conversation list redraws the conversation beside it, a kilobyte or two per key for text. A photo preview is the expensive part: about 100 KB as sixel, sent again whenever the conversation shifts under it (a new message, a longer draft). In the list, a conversation's photos go out only once the highlight has rested on it for 150 ms, so moving past it quickly sends none. When the terminal takes more than 250 ms to answer at startup, as over a slow or distant link, previews start as colored blocks (about 8 KB) instead. `TUIMSG_IMAGES=sixel` brings photos back; `TUIMSG_IMAGES=blocks` keeps blocks on any link. Animated GIFs keep redrawing while they are on screen in block mode.

imsg answers every request from one database connection, one query at a time, so `tuimsg` keeps its own background work out of the way. It lists the first 60 conversations first, then the rest. It fetches each conversation's latest message for the list one at a time, and pauses that whenever a conversation you open or highlight, or a message you send, is waiting on imsg. Highlighted conversations load one at a time too; moving faster than they load skips to wherever the highlight is when the last one arrives. For merged conversations, which can switch between iMessage and SMS, it checks which service the latest message used for the 40 at the top of the list. The others are checked when shown or when a message arrives.
...
| Context | Keys | Action |
|---|---|---|
| Conversation list | `j` / `k`, arrows | Move the highlight, showing that conversation |
